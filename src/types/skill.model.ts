...
const SKILLS_N_PATH: (skills: Skill[]) => SkillAndPath[] = (skills: Skill[]) => {
...
export const SKILLS_N_PATH_BY_GROUP: (skills: Skill[]) => SkillByGroup[] = (skills: Skill[]) => {
  const skills_n_path = SKILLS_N_PATH(skills)

  return [...new Set(skills_n_path.map(item => item.type))].map(group => {
    return {
      group: group!,
